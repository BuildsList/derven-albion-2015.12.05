	...
	if(GLOBAL_RADIO_TYPE == 1) // NEW RADIO SYSTEMS: By Doohl
	   ...
	   //#### Grab the connection datum ####//
		...
		if (ishuman(M))
			...
			var/voice = M.gender_word + " " + M.age_word // Why reinvent the wheel when there is a proc that does nice things already
	  ...
	  /* ###### Radio headsets can only broadcast through subspace ###### */
		if(M.age < 20)
			M.age_word = "\[Юность\]"
		else if(M.age > 20 && M.age < 40)
			M.age_word = "\[Зрелость\]"
		else if(M.age > 40)
			M.age_word = "\[Cтарость\]"

		if(M.gender == "male")
			M.gender_word = "\[Мужчина\]"
		else
			M.gender_word = "\[Женщина\]"

		var/NAME_FOR_YOU = "[M.gender_word] [M.age_word]"
		...
		if(subspace_transmission)
			...
			signal.data = list(
			  // Identity-associated tags:
				...
				"realname" = NAME_FOR_YOU, // the mob's real name
				"name" = NAME_FOR_YOU,	// the mob's display name
				"job" = jobname,		// the mob's job
				"key" = mobkey,			// the mob's key
				"vmessage" = M.voice_message, // the message to display if the voice wasn't understood
				"vname" = NAME_FOR_YOU, // the name to display if the voice wasn't understood
	  ...
	  /* ###### Intercoms and station-bounced radios ###### */
		...
		/* --- Try to send a normal subspace broadcast first */
				...
				//rendered = "<span class='game say'><span class='name'>\[[NAME_FOR_YOU]\] </span><span class='message'>[message_a]</span></span>"
		if(M.age < 20)
			M.age_word = "\[Юность\]"
		else if(M.age > 20 && M.age < 40)
			M.age_word = "\[Зрелость\]"
		...
		NAME_FOR_YOU = "[M.gender_word] [M.age_word]"
		...
		if (length(heard_masked) || length(heard_normal) || length(heard_voice) || length(heard_garbled))
			var/part_a = "<span class='radio'><span class='gender'>"
			...
			var/blackbox_msg = "[part_a]\[[M.gender]][part_blackbox_b][quotedmsg][part_c]"
			...
			if (length(heard_normal))
				...
				for (var/mob/R in heard_normal)
					if(istype(R, /mob/living/silicon/ai))
						R.show_message("[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>\[[M.gender]] </a>[part_b][quotedmsg][part_c]", 2)
					else
						R.show_message(rendered, 2)
			...
			if (length(heard_voice))
				...
				for (var/mob/R in heard_voice)
					if(istype(R, /mob/living/silicon/ai))
						R.show_message("[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>\[[M.gender]] </a>[part_b][M.voice_message][part_c]", 2)
					else
						R.show_message(rendered, 2)
