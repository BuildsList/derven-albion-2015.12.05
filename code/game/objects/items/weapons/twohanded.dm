...
/obj/item/weapon
	...
	New()
		...
		force_wielded = force + round(force/3)

/obj/item/weapon/proc/unwield()
	if(twohanded == 1)
		wielded = 0
...
/obj/item/weapon/required/
	w_class = 5.0
	twohanded = 1

/obj/item/weapon/required/attack_self()
	return

/obj/item/weapon/required/mob_can_equip(M as mob, slot)
	if(wielded)
		M << "<span class='warning'>[src.name] is too cumbersome to carry with anything but your hands!</span>"
		return 0
	return ..()

/obj/item/weapon/twohanded/attack_hand(mob/user)//Can't even pick it up without both hands empty
