...
*/
	...
	if (length(heard_a))
		...
		if(age < 20)
			age_word = "[format]\[Юность\][close_format]"
		else if(age > 20 && age < 40)
			age_word = "[format]\[Зрелость\][close_format]"
		else if(age > 40)
			age_word = "[format]\[Cтарость\][close_format]"

		if(gender == "male")
			gender_word = "[format]\[Мужчина\][close_format]"
		else
			gender_word = "[format]\[Женщина\][close_format]"

//		rendered = "<span class='game say'><span class='name'>[GetVoiceName()]</span><span class='message'>[message_a]</span></span>"
		rendered = "<span class='game say'><span class='name'>[gender_word] [age_word]</span><span class='message'>[message_a]</span></span>"
			...
			if(hascall(M,"show_message"))
				...
				if(istype(M, /mob/living/carbon/human))
					...
					if(M in view(2,usr))
						...
						close_format = "</font>"
					H = M
					for(var/datum/data_name/D in name_list)
						if(D.name == H.real_name)
