/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M)
	...
	switch(M.a_intent)
		...
		if("grab")
			...
			if(buckled)
				M << "<span class='notice'>Вы не можете схватить [src], он(а) пристегнут(а) к стулу!</span>"
			...
			if(M.zone_sel.selecting == "mouth")
				...
				GRABMOUTH = 1
				visible_message("<span class='warning'>[M] хватает [src] и закрывает рот руками!</span>")
				return 1
			else
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
